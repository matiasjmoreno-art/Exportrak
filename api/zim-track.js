// api/zim-track.js
// Client ID y Secret del email de bienvenida de ZIM (produccion)
...
async function getToken() {
  ...
  const TOKEN_URLS = [
    ...
    "https://zimidapimprod.b2clogin.com/zimidapimprod.onmicrosoft.com/B2C_1A_SIGNUP_SIGNIN/oauth2/v2.0/token",
  ...
  for (const url of TOKEN_URLS) {
    try {
      ...
      tokenDebug.push({ url: url.slice(-60), status: r.status, body: txt.slice(0, 300) });
      if (r.ok) {
      ...
      tokenDebug.push({ url: url.slice(-60), error: e.message });
    }
  }
  return null;
}
